...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        nstate = []
        for condition in action.pc:             # EX1: Verficar as pré-condições da ação e só aceita se estiverem/coincidirem com o estado atual
            if condition not in state:
                return None
        for condition in state:                 #      Subtrair efeitos negativos
            if condition not in action.neg:
                nstate.append(condition)
        for result in action.pos:               #      Adicionar efeitos positivos
    ...
    def satisfies(self, state, goal):           #      Verifica se as condições do objetivo estão no estado atual
