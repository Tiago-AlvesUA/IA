...
class Cidades(SearchDomain):
    ...
    def result(self,city,action):
        ...
        if C1==city:
            return C2
    def cost(self, city, action):                               # EXC7: 
        if city != action[0]:
            return None
        for (C1,C2,D) in self.connections:                      
            if C1 == action[0] and C2 == action[1]:             #       Se action[0] for a cidade que pretendemos buscamos a distância 
                return D                                        #       entre esta e a outra (action[1])
            if C2 == action[0] and C1 == action[1]:
                return D
    def heuristic(self, city, goal_city):                       # EXC11
        coord_city = self.coordinates[city]
        coord_goal_city = self.coordinates[goal_city]
        return math.sqrt((coord_city[0]-coord_goal_city[0])**2 + (coord_city[1]-coord_goal_city[1])**2)
